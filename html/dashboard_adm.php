    ...
    <aside class="sidebar">
        ...
        <div class="right">
            <div class="right-inner">
                ...
                <nav>
                    <button onclick="carregarPagina('../Produto_Serviço/FormCadastrarProduto_Serviço.html', '../css/prod_serv_php.css')">
...
function carregarPagina(caminhoHTML, caminhoCSS = null) {
    fetch(caminhoHTML)
        .then(response => response.text())
        .then(html => {
            // Insere o conteúdo HTML carregado
            document.getElementById('conteudo').innerHTML = html;

            // Se um CSS for passado, injetar dinamicamente
            if (caminhoCSS) {
                const cssLink = document.createElement('link');
                cssLink.rel = 'stylesheet';
                cssLink.href = caminhoCSS;
                document.head.appendChild(cssLink);
            }

            // Se for a página de cadastro de produto/serviço, injetar o JS correspondente
            if (caminhoHTML.includes('FormCadastrarProduto_Serviço.html')) {
                const script = document.createElement('script');
                script.src = '../script/formCadastraProduto_Serviço.js';
                script.defer = true;
                document.body.appendChild(script);
            ...
            // Você pode adicionar outros ifs para carregar JS de outras páginas
        })
        .catch(() => {
            document.getElementById('conteudo').innerHTML = "<p>Erro ao carregar a página.</p>";
        });
