...
CREATE TABLE IF NOT EXISTS tb_estoque_atualizacao (
    ...
    UNIQUE KEY unique_produto (tb_produtos_id_produtos),
...
FOR EACH ROW
BEGIN
    INSERT INTO tb_vendas (data_venda, valor) VALUES (NEW.data_compra, NEW.valor_total);
END;//
DELIMITER ;
...
INSERT INTO tb_vendas (data_venda, valor) VALUES
(CURDATE(), 125.75);
-- daqui pra baixo aplicar tudo por favor não pergunte aplique


ALTER TABLE tb_clientes
ADD COLUMN cpf VARCHAR(14) NOT NULL AFTER nome;

SELECT * FROM tb_clientes;
DESCRIBE tb_clientes;

ALTER TABLE tb_clientes ADD COLUMN ativo TINYINT(1) DEFAULT 1;
SELECT nome, email, ativo FROM tb_clientes;
SELECT id_cliente, nome, email, ativo FROM tb_clientes WHERE email = 'romulo@gmail.com';
DESCRIBE tb_clientes;

SELECT id_clientes, nome, email, ativo 
FROM tb_clientes 
WHERE email = 'romulo@gmail.com';
SELECT * FROM tb_usuarios_login WHERE email = 'romulo@gmail.com';

DELETE FROM tb_clientes
where id_clientes in (7,8,9,10);

SHOW COLUMNS FROM tb_login_gerencia;
ALTER TABLE tb_clientes MODIFY senha VARCHAR(255);
...
CREATE TABLE IF NOT EXISTS tb_login (
    ...
    tipo_usuario ENUM('cliente', 'cabeleleira', 'atendente', 'adm') NOT NULL,
    id_referencia INT DEFAULT NULL, -- pode ser usado para associar com a tabela tb_clientes, tb_funcionarios etc.
    status TINYINT(1) NOT NULL DEFAULT 1, -- 1 = ativo, 0 = inativo
    data_criacao DATETIME DEFAULT CURRENT_TIMESTAMP
);
-- Adiciona a coluna tipo_funcionario na tabela tb_funcionarios
