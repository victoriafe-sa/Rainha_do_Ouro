-- Desativa verificações para evitar erros na criação
SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION';

-- Cria schema se não existir e usa ele
CREATE SCHEMA IF NOT EXISTS `db_rainhadoouro`;
USE `db_rainhadoouro`;

-- Tabelas originais (mantidas sem alterações)
...
CREATE TABLE IF NOT EXISTS `tb_historico_agendamento` (
...
-- -----------------------------------------------------
-- NOVA TABELA PARA LOGIN UNIFICADO
-- -----------------------------------------------------
-- A tabela `tb_usuarios_login` foi criada para centralizar as credenciais
-- de acesso dos usuários do sistema, independentemente do tipo (admin, secretaria,
-- cliente, cabeleireiro). Ela guarda email, senha (criptografada), tipo de usuário,
-- um id referenciando o registro original em outras tabelas, e se o usuário está ativo.
-- Essa abordagem facilita o controle de permissões e autenticação de forma única.
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `tb_usuarios_login` (
  `id_usuario` INT NOT NULL AUTO_INCREMENT,
  `email` VARCHAR(100) NOT NULL UNIQUE,
  `senha` VARCHAR(255) NOT NULL,
  `tipo_usuario` ENUM('admin', 'secretaria', 'cliente', 'cabeleireiro') NOT NULL,
  `id_externo` INT, -- ID correspondente na tabela original, ex: tb_clientes.id_clientes
